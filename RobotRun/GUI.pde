...
public void addNumber(String number) {
  ...
  else if(mode == Screen.SET_MV_INSTRUCT_SPEED) {
    workingText += number;
  ...
  switch(mode) {
    case INSTRUCTION_NAV:
      if(shift == ON) {
        ...
        nextScreen(Screen.PICK_INSTRUCTION);
      }
      break;
    case NAV_TOOL_FRAMES:
      if(shift == ON) {
        // Reset the highlighted frame in the tool frame list
        toolFrames[opt_select] = new ToolFrame();
    ...
    case NAV_USER_FRAMES:
      if(shift == ON) {
        ...
        userFrames[opt_select] = new UserFrame();
        saveFrameBytes( new File(sketchPath("tmp/frames.bin")) );
        updateScreen();
      } else {
  ...
  case INSTRUCTION_NAV:
    ...
    if(ins instanceof MotionInstruction) {
      ...
      case 3: // register type
        ...
        break;
      case 4: // register
        nextScreen(Screen.SET_MV_INSTRUCT_REG_NUM);
        break;
      case 5: // speed
        nextScreen(Screen.SET_MV_INSTRUCT_SPEED);
        break;
      case 6: // termination type
        nextScreen(Screen.SET_MV_INSTRUCT_TERMINATION);
        break;
      }
    } 
    break;
    ...
    case INSTRUCT_MENU_NAV:
      switch(opt_select) {
        ...
        case 6: //Comment
          p = programs.get(active_prog);
          selectedLines = resetSelection(p.getInstructions().size());
          nextScreen(Screen.SELECT_COM_UNCOM);
        case 7: //Undo
        case 8: //Remark
      }
      break;
...
public ArrayList<String> getOptions(Screen mode){
  ...
  switch(mode) {
    ...
    case VIEW_INST_REG:
      ...
      break;
    case NAV_TOOL_FRAMES:
      options = loadFrames(CoordFrame.TOOL);
      break;
    case NAV_USER_FRAMES:
 ...
 */
public ArrayList<String> loadDirectEntryMethod() {
  if (teachFrame != null) {
    ...
    frame.add(String.format("X: %4.3f", xyz.x));
    frame.add(String.format("Y: %4.3f", xyz.y));
    frame.add(String.format("Z: %4.3f", xyz.z));
    frame.add(String.format("W: %4.3f", wpr.x * RAD_TO_DEG));
    frame.add(String.format("P: %4.3f", wpr.y * RAD_TO_DEG));
    frame.add(String.format("R: %4.3f", wpr.z * RAD_TO_DEG));
    
    return frame; 
  }
  
  println("Error: input frame is null!");
  return null;
}
...
public void createFrameDirectEntry() {
  boolean error = false;
